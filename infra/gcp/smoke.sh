...
#   TIMEOUT      (tuỳ)      giây/mỗi request (mặc định 20).
...
run() {  # label  expect_code  method  url  [curl-args...]
  ...
  local code
  code="$(curl -sS -m "${TIMEOUT:-20}" -o "$resp" -w '%{http_code}' -X "$method" "$url" "$@" 2>>"$resp")" || code="000"
  if [[ "$code" == "$expect" ]]; then
...
if [[ -n "${API_KEY}" ]]; then
  ...
  run "/v1/jobs/search" 200 POST "${BASE_URL}/v1/jobs/search" \
    "${BEAR[@]}" "${KEY[@]}" -H "Content-Type: application/json" \
