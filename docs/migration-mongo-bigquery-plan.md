...
1. **Versioned + metadata bất biến (hai bảng):**
   - `silver_jobs`/`gold_market_metrics` mang cột `batch_id`, ELT **APPEND**; work table (candidate) đặt tên **RIÊNG theo run** (`*_candidate__<batch_id_sạch>_<random>`, `WRITE_TRUNCATE` rồi **DROP**; KHÔNG dùng chung — tránh hai run song song ghi đè nhau), KHÔNG phải dataset **môi trường** staging. Ghi published bằng **DELETE-by-batch_id + INSERT-từ-candidate TRONG cùng transaction** với CAS+catalog (run thua CAS rollback cả dữ liệu → không rows trùng). Giữ **≥2 batch**. ("staging" = môi trường `jobs_staging`/`jobs_prod`; "candidate" = work table ETL tạm.)
   - **`warehouse_state`** (singleton pointer): `warehouse_name, published_batch_id`. Seed 1 row `published_batch_id=NULL` **trong publish transaction** (INSERT…WHERE NOT EXISTS trước CAS) → hai first-publish đồng thời không tạo 2 dòng.
...
2. **Idempotency + concurrency + publish nguyên tử:** logical key silver `(batch_id, job_id)`, gold `(batch_id, window, dimension, dimension_value)`. **Retry = state machine theo "có mặt trong `warehouse_batches`" (= đã publish, BẤT BIẾN):**
   ...
   **`warehouse_batches` INSERT + `warehouse_state` CAS trong CÙNG transaction** (`BEGIN…COMMIT`); `published_at` sinh trong transaction; CAS `WHERE published_batch_id=@expected_prev` (đầu tiên `IS NULL`); **assert affected==1 NGAY SAU UPDATE** (≠1→ROLLBACK toàn bộ). Silver/gold/quarantine **DELETE-by-batch_id + INSERT-từ-candidate NẰM TRONG transaction** (run thua CAS rollback cả dữ liệu → không rows trùng); seed singleton cũng trong txn. CLI: `BATCH_ALREADY_PUBLISHED`→exit non-zero có kiểm soát, không auto-retry vô hạn. Transaction abort do concurrent mutation → đọc lại state, áp state machine, không retry mù `expected_prev` mới.
...
- Load **candidate tables per-batch** (`WRITE_TRUNCATE`, drop sau) → **quality checks** → **build gold TỪ validated candidate** (90d + all_time; `CROSS JOIN UNNEST(categories)` + dedupe `(batch_id,job_id,category_key)` + `COUNT(DISTINCT job_id)`; unknown bucket) → **transaction: seed singleton + DELETE-by-batch_id + INSERT silver/gold/quarantine từ candidate + INSERT `warehouse_batches` + CAS `warehouse_state`** (tất cả nguyên tử). Gold build từ candidate để né `require_partition_filter` + cùng snapshot.
